...
public class RerouteTest {
    ...
    public void testMoveReplicaServerReroutingOff() throws Exception {
        try {
            ...
            // single get
            Assert.assertThrows(RuntimeException.class, () -> moveClient.get(testInt32RerouteTableName, new Object[] {0}, new String[] {"c2"}));

            // multi get
            TableBatchOps tableBatchOps = moveClient.batch(testInt32RerouteTableName);
            tableBatchOps.get(new Object[] {0}, new String[] {"c2"});
            Assert.assertThrows(RuntimeException.class, tableBatchOps::execute);

            // query
            TableQuery tableQuery = moveClient.query(testInt32RerouteTableName).select("c1", "c2").addScanRange(new Object[] {0}, new Object[] {0});
            QueryResultSet rs;
            Assert.assertThrows(RuntimeException.class, tableQuery::execute);

            // update
            Assert.assertThrows(RuntimeException.class, () -> moveClient.update(testInt32RerouteTableName, new Object[]{0}, new String[]{"c2"}, new Object[]{5}));
        } finally {
            setReroutingEnable(true);
            Thread.sleep(1000);
            moveClient.delete(testInt32RerouteTableName, new Object[] {0});
