...
/*
 * 表模型6：普通分区堆表 + 唯一二级索引(c1) + 自增列 + 默认更新时间 + 唯一二级索引(c1, c2) + 全局二级索引(c1) + 生成列 + TTL
 * CREATE TABLE test_heap_model7 (
 * c1 bigint, 
 * c2 varchar(20), 
 ...
 * UNIQUE KEY idx_unique_c1 (c1) GLOBAL,
 * UNIQUE KEY idx_unique_c1_c2 (c1, c2)
 * ) ORGANIZATION = HEAP TTL(c5 + INTERVAL 100 SECOND) PARTITION BY KEY(c1) PARTITIONS 3;
 */
public class ObHeapTableModel7Test {
    ObTableClient        client;
    public static String tableName = "test_heap_model7";
