...
/*
 * 表模型5：普通分区堆表 + 唯一二级索引(c1) + 自增列 + 默认更新时间
 * CREATE TABLE test_heap_model5 (
 * c1 bigint, 
 * c2 varchar(20), 
 ...
 */
public class ObHeapTableModel5Test {
    ObTableClient        client;
    public static String tableName = "test_heap_model5";
    ...
    private void createTable() throws Exception {
        ...
        statement.execute("CREATE TABLE " + tableName + " ( " + "c1 bigint, " + "c2 varchar(20), "
                          + "c3 bigint auto_increment, " + "c4 bigint, "
                          + "c5 timestamp default current_timestamp on update current_timestamp, "
                          + "c6 varchar(20), " + "UNIQUE KEY idx_unique_c1 (c1), "
                          + "UNIQUE KEY idx_unique_c1_c2 (c1, c2) "
                          + ") ORGANIZATION = HEAP PARTITION BY KEY(c1) PARTITIONS 3");
    }
    ...
    public void testSingleInsert2() throws Exception {
        try {
            ...
            assertTrue(thrown.getMessage().contains("[-5024][OB_ERR_PRIMARY_KEY_DUPLICATE][Duplicate entry '1001' for key 'idx_unique_c1']"));
        } finally {
            // 清理数据
            truncateTable();
        }
