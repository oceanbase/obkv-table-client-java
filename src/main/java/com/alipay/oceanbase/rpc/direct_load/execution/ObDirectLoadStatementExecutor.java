...
public class ObDirectLoadStatementExecutor {
    ...
    public synchronized ObDirectLoadStatementFuture requestAbort() {
        ObDirectLoadStatementFuture task = null;
        final int state = stateFlag.get();
        try {
            if (NodeRole.PRIMARY != nodeRole && NodeRole.P2P != nodeRole) {
            ...
            } else if (NONE == state || BEGINNING == state) {
                String message = "abort is not allowed, because state is " + state;
                logger.warn(message);
                throw new ObDirectLoadIllegalStateException(message);
            } else {
                if (this.abortFuture != null) {
                    logger.debug("already in abort process");
                    task = abortFuture;
                } else {
                    task = abort();
                }
            ...
            logger.warn("statement request abort failed", e);
            return new ObDirectLoadStatementFailedFuture(statement, e);
        }
        return task;
    }
