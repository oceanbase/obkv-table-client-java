...
import com.alipay.oceanbase.rpc.protocol.payload.impl.parser.gen.ObExprGrammerLexer;

public class ObGeneratedColumnExpressParser {

    private ObExprGrammerParser parser;

    /*
     * Ob generated column express parser.
     */
    public ObGeneratedColumnExpressParser(String text) {
        ObExprGrammerLexer lexer = new ObExprGrammerLexer(CharStreams.fromString(text));
        CommonTokenStream tokens = new CommonTokenStream(lexer);
        parser = new ObExprGrammerParser(tokens);
    }
    ...
    public ObGeneratedColumnSimpleFunc parse() throws GenerateColumnParseException {
        ObExprGrammerParser.ExprContext ctx = parser.expr();
        return new ObGeneratedColumnSimpleFunc(ctx);

        //        return parser.expr().v;
        //        switch (lexer.token()) {
        //            case IDENTIFIER:
