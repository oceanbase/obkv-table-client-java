...
public class ObTableMoveReplicaInfo extends AbstractPayload {
    ...
    public byte[] encode() {
        ...
        System.arraycopy(Serialization.encodeVi64(tableId), 0, bytes, idx, Serialization.getNeedBytes(tableId));
        System.arraycopy(Serialization.encodeVi64(schemaVersion), 0, bytes, idx, Serialization.getNeedBytes(schemaVersion));
        if (OB_VERSION >= 4){
            System.arraycopy(Serialization.encodeI64(partitionId), 0, bytes, idx, Serialization.getNeedBytes(partitionId));
        } else {
            System.arraycopy(Serialization.encodeVi64(partitionId), 0, bytes, idx, Serialization.getNeedBytes(partitionId));
        }
        System.arraycopy(server.encode(), 0, bytes, idx, Serialization.getNeedBytes(server.encode()));
        System.arraycopy(Serialization.encodeVi32(obServerRole.getIndex()), 0, bytes, idx, Serialization.getNeedBytes(obServerRole.getIndex()));
        System.arraycopy(Serialization.encodeVi32(obReplicaType.getIndex()), 0, bytes, idx, Serialization.getNeedBytes(obReplicaType.getIndex()));
        System.arraycopy(Serialization.encodeVi64(partRenewTime), 0, bytes, idx, Serialization.getNeedBytes(partRenewTime));
