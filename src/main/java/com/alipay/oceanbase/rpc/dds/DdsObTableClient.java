...
public class DdsObTableClient extends AbstractTable implements OperationExecuteAble, Lifecycle {
    ...
    */
    ...
    public TableEntry getOrRefreshTableEntry(final String tableName, final Integer groupID, final boolean refresh,
        ...
        try {
            obTableClient.getOrRefreshTableEntry(tableName, refresh);
        } catch (Throwable t) {
    ...
    public TableEntry getOrRefreshTableEntry(final String tableName, final boolean refresh,
        ...
        for (Map.Entry<String, ObTableClient> entry: atomDataSources.entrySet()) {
            try {
                entry.getValue().getOrRefreshTableEntry(tableName, refresh);
            } catch (Throwable t) {
                logger.error("getOrRefreshTableEntry execption", t);
            }
        }
    ...
    /**
     * Execute ObHbaseRequest
     * @param request ObHbaseRequest
     * @return ObPayload
     * @throws Exception if fail
     */
    @Override
    public ObPayload execute(ObHbaseRequest request) throws Exception {
        checkStatus();
        ...
        // For HBase requests, we need to extract row key information from the request
        // Similar to ObTableClient, we use the first cell from the first cfRows to route
        if (request.getCfRows().isEmpty()) {
            throw new ObTableUnexpectedException("no cf rows");
        }

        // Extract row key information for routing
        // Use the first key from the request for routing
        if (request.getKeys().isEmpty()) {
            throw new ObTableUnexpectedException("no keys in HBase request");
        }

        // Get the first key for routing
        int keyIdx = request.getCfRows().get(0).getKeyIndex(0);
        Object[] rowKey = {request.getKeys().get(keyIdx).getValue()};

        // Calculate database and table based on the row key
        DatabaseAndTable databaseAndTable = calculateDatabaseAndTable(tableName, rowKey, false);
        ...
        // Update the table name in the request to the target table name
        request.setTableName(getTargetTableName(databaseAndTable.getTableName()));
    ...
    public ObTableServerCapacity getServerCapacity() {
        // get random table client and return its server capacity
        return atomDataSources.values().stream().findAny().orElse(null).getServerCapacity();
    }
}
