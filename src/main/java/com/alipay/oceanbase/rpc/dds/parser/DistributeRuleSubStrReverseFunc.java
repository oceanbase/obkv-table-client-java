 ...
 */
public class DistributeRuleSubStrReverseFunc implements DistributeRuleSimpleFunc {
    ...
    public void setParameters(List<Object> parameters) throws IllegalArgumentException {
        if (parameters.size() < 2) {
            throw new IllegalArgumentException(
                "substr reverse argument size must be bigger than 2 " + parameters);
        }
        ...
        if (!(parameter1 instanceof String)) {
            throw new IllegalArgumentException(
                "substr reverse first argument must be column or string " + parameter1);
        }
        ...
        if (!((parameter2 instanceof Long || (parameter2 instanceof Integer)))) {
            throw new IllegalArgumentException("substr reverse second argument pos must be int "
                                               + parameter2);
        }
        ...
        if (parameters.size() == 3) {
            ...
            if (!((parameter3 instanceof Long) || (parameter3 instanceof Integer))) {
                throw new IllegalArgumentException("substr reverse third argument len must be int "
                                                   + parameter3);
            }
            ...
            if (len <= 0 || len > Integer.MAX_VALUE) {
                throw new IllegalArgumentException(
                    "substr reverse third argument len must be positive int");
            }
    ...
    public Object evalValue(List<Object> refs) throws IllegalArgumentException {
        ...
        if (refs.size() != refColumnNames.size()) {
            throw new IllegalArgumentException("DistributeRuleSubStrFunc is refer to "
                                               + refColumnNames
        ...
        if (pos > 0) {
            if (pos > evalStrLen) {
                ...
                if (len > 0 && pos - 1 + len <= evalStrLen) {
                    return new StringBuffer(evalStr.substring(pos - 1, pos - 1 + len)).reverse()
                        .toString();
                }
                return new StringBuffer(evalStr.substring(pos - 1)).reverse().toString();
            }
            ...
            if (-pos > evalStrLen) {
                ...
                if (len > 0 && pos + len <= evalStrLen) {
                    return new StringBuffer(evalStr.substring(pos, pos + len)).reverse().toString();
                }
                return new StringBuffer(evalStr.substring(pos)).reverse().toString();
            }
        }
    ...
    public String toString() {
        return "DistributeRuleSubStrFunc{" + "refColumnNames=" + refColumnNames + ", pos=" + pos
               + ", len=" + len + '}';
    }
}
